...
class RegistrationRestControllerTest {
    ...
    void getById() throws Exception {
        RegistrationDTO dto = createTestRegistrationDto();
        when(service.getRegistrationById(ID)).thenReturn(dto);

        mockMvc.perform(get(AUDIT_BY_ID_URL, ID))
                .andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(dto)));

        verify(service, times(1)).getRegistrationById(ID);
    }

    @Test
    void getAllAudit() throws Exception {
        RegistrationDTO dto = createTestRegistrationDto();
        List<RegistrationDTO> dtoList = Collections.singletonList(dto);
        when(service.getAllRegistration()).thenReturn(dtoList);

        mockMvc.perform(get(ALL_AUDITS_URL))
                .andExpect(status().isOk())
                .andExpect(content().json(objectMapper.writeValueAsString(dtoList)));

        verify(service, times(1)).getAllRegistration();
    }
